...
import { CreditCard, GraduationCap, UserCheck, LogOut } from 'lucide-react';
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export default function CashierSidebar() {
  ...
  const activeSection = searchParams.get('section') ?? 'payments';
          ...
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">
              {NAV_ITEMS.map(({ label, section, icon: Icon, accent, activeBg }) => {
                const href = `/cashier/dashboard?section=${section}`;
        ...
        {collapsed ? (
          <div className="flex justify-center mb-2 py-1">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #B8FF8F 0%, #34A0C5 100%)', color: '#0e1a2a' }}
              aria-hidden="true"
            >
              C
            </div>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold text-white truncate leading-none mb-1">Cashier</p>
              <span
                className="text-[10px] px-1.5 py-0.5 rounded-full font-semibold leading-tight"
                style={{
                  background: 'rgba(5,58,52,0.4)',
                  color: '#B8FF8F',
                  border: '1px solid rgba(184,255,143,0.25)',
                }}
              >
                Cashier
              </span>
